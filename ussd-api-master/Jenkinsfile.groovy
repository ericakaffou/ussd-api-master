...
node(){
  try{
    ...
    /*
      ...
      echo "Build ignoré sur la branche stage"
  ...
  // Ajout du nom du conteneur dans application properties //
    ...
    def version = sh returnStdout: true, script: "cat pom.xml | grep -A1 '<artifactId>myapp1' | tail -1 |perl -nle 'm{.*<version>(.*)</version>.*};print \$1' | tr -d '\n'"
     ...
     // le docker run n'utilise pas le Dockerfile contenu dans le rep.
     //docker run --rm --name maven-${commitIdLong} telecharge une image maven et la renomme maven-${commitIdLong}.
      // Maven - tests //
    ...
    def imageName='192.168.1.15:5000/myapp'
    // ici utilisation du Dockerfile
    stage('DOCKER - Build/Push registry'){
      docker.withRegistry('http://192.168.1.15:5000', 'myregistry_login') {
    ...
    stage('DOCKER - check registry'){
      ...
      sh 'curl -sk --user $USERNAME:$PASSWORD http://192.168.1.15:5000/v2/myapp/tags/list'
      }
    }
